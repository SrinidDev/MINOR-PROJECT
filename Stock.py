...
def main():
    ...
    df = pd.read_excel('https://github.com/SrinidDev/MINOR-PROJECT/blob/master/ticker.xlsx')
...
def portfolio(start, end, df, mydict):
    ...
    if (cnames):
        ...
        colsmax = st.beta_columns(len(max_return))
        colsmin = st.beta_columns(len(min_volatility))
